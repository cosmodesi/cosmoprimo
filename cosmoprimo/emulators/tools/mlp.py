...
class MLPEmulatorEngine(BaseEmulatorEngine):
    ...
    def _fit_no_operation(self, X, Y, attrs, validation_frac=0.1, optimizer='adam', loss=None, batch_frac=(0.1, 0.3, 1.), epochs=1000, learning_rate=(1e-2, 1e-3, 1e-5), patience=100, verbose=0, seed=None):
        """
        Fit.
        ...
        class TFModel(tf.keras.Model):
            ...
            def __init__(self, architecture):
                ...
                for i in range(self.nlayers):
                    ...
                    self.W.append(tf.Variable(tf.random.normal([self.architecture[i], self.architecture[i + 1]], 0., 1e-3), name='W_{:d}'.format(i), trainable=True))
    ...
    def _fit_no_operation2(self, X, Y, attrs, validation_frac=0.1, optimizer='adam', loss=None, batch_frac=(0.1, 0.3, 1.), epochs=1000, learning_rate=(1e-2, 1e-3, 1e-5), patience=100, verbose=0, seed=None):
        """
        Fit.
