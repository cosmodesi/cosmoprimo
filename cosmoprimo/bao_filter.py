...
class BasePowerSpectrumBAOFilter(BaseClass):
...
class Wallish2018PowerSpectrumBAOFilter(BasePowerSpectrumBAOFilter):
    ...
    def compute(self):
        """Run filter."""
        from scipy import fftpack, ndimage, interpolate
        ...
        dd_odd = spline_even(xodd,nu=2)
        self._even = even # if one wants to check everything is ok
        self._odd = odd
        ...
        mask_left = self.k < 1e-3
        k = np.concatenate([self.k[mask_left],k])
        pknow = np.concatenate([self.pk[mask_left],pknow])
        pknow_interpolator = self.pk_interpolator.clone(k=k,pk=pknow)
        self.pknow = pknow_interpolator(self.k)
        """
        mask = (k > 1e-2) & (k < 1.5)
        k, pknow = k[mask], pknow[mask]
        ...
        self.pknow = interpolate.CubicSpline(k,pknow,axis=0,bc_type='clamped',extrapolate=False)(self.k)
        tophat = self._tophat(self.k,kmax=1.,scale=20.)
        wiggles = (self.pk/self.pknow - 1.).T*tophat + 1.
        self.pknow = self.pk/wiggles.T
...
class BaseCorrelationFunctionBAOFilter(BaseClass):
...
power_spectrum_bao_filters = {'hinton2017':Hinton2017PowerSpectrumBAOFilter,'ehpoly':EHNoWigglePolyPowerSpectrumBAOFilter,
                            'savgol':SavGolPowerSpectrumBAOFilter,'wallish2018':Wallish2018PowerSpectrumBAOFilter}
correlation_function_bao_filters = {'kirkby2013':Kirkby2013CorrelationFunctionBAOFilter}


def PowerSpectrumBAOFilter(pk_interpolator, engine='savgol', **kwargs):
    """Run power spectrum BAO filter corresponding to the provided engine."""
    if engine in power_spectrum_bao_filters:
        return power_spectrum_bao_filters[engine](pk_interpolator,**kwargs)
    raise ValueError('Power spectrum BAO filter {} is unknown'.format(engine))


def CorrelationFunctionBAOFilter(xi_interpolator, engine='savgol', **kwargs):
    """Run correlation function BAO filter corresponding to the provided engine."""
    if engine in correlation_function_bao_filters:
        return correlation_function_bao_filters[engine](xi_interpolator,**kwargs)
